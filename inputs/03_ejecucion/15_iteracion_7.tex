...
planificadas. Dos de ellas, Sistema de Combate II y Sistema de Enemigos II, provenían de una
replanificación deliberada tras comprobar en la iteración 5 que su complejidad real era
mayor de la estimada inicialmente. En consecuencia, el control en esta fase no consistió solo
en abrir trabajo nuevo, sino en cerrar con más profundidad aspectos que ya se habían validado
como base funcional.

Desde el punto de vista del tiempo, esta iteración sí muestra una desviación más clara:
frente a las 50:00 horas planificadas, se registraron 57:14 horas reales. Esa diferencia
encaja con el hecho de que el refinamiento de combate y enemigos absorbió más esfuerzo y con
que la parte de salas especiales quedó estructuralmente resuelta, pero no completamente
cerrada desde el punto de vista funcional.

Por tanto, el seguimiento y control de la iteración lleva a una conclusión clara: la división
previa de features fue necesaria y acertada, pero aun así algunas tareas siguieron mostrando
una complejidad superior a la esperada. El alcance principal sobre combate y enemigos sí
puede considerarse consolidado, mientras que tienda y salas especiales justifican una
replanificación parcial del cierre para no presentar como terminada una funcionalidad que aún
requería remate efectivo.
...
\begin{table}[h!]
    ...
    \midrule
    Sala Tienda & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Generar varias mazmorras hasta localizar la sala de tienda. \item Acceder a ella desde el recorrido normal y observar su representación en el minimapa. \item Comprobar que aparece como sala terminal diferenciada, que su tipo se conserva en la celda y que puertas y muros se configuran de forma coherente.\end{enumerate}\end{minipage} \\
    \midrule
    Sala Secreta y Tesoro & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Generar partidas con salas especiales activas. \item Localizar la sala de tesoro y buscar la sala secreta mediante exploración del mapa. \item Verificar que ambas usan iconografía propia, que la sala secreta no queda conectada como una habitación regular visible y que su colocación no rompe la navegación del nivel.\end{enumerate}\end{minipage} \\
    \midrule
    Sistema de Combate II & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Probar armas con ataque normal y ataque cargado en encuentros reales. \item Observar trazos de melé, indicadores de carga, proyectiles y efectos visuales asociados. \item Confirmar que el \textit{feedback} acompaña al momento del ataque, que el daño se aplica al finalizar la acción y que el contexto del arma se mantiene durante el impacto.\end{enumerate}\end{minipage} \\
    \midrule
    Sistema de Enemigos II & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Entrar en salas con enemigos normales, voladores y tanque. \item Mantener distintas distancias para provocar carga, disparo, persecución y recuperación tras empuje. \item Verificar que los avisos previos al ataque son legibles, que cada arquetipo mantiene su patrón táctico y que los multiplicadores de daño responden al arma usada por el jugador.\end{enumerate}\end{minipage} \\
    \end{coolTable}
