...
Siguiendo el enfoque de \textit{Feature Driven Development} (FDD) \cite{palmer2002fdd}, en esta iteración se
...
\item \textbf{Jugador.} Esta funcionalidad convirtió por primera vez el nivel
...
\item \textbf{Enemigos.} Esta funcionalidad incorporó oposición activa dentro del
...
\subsection{Diseño de Generación Procedural}
...
introducir oposición funcional sin comprometer la estabilidad del arranque \cite{gamma1994designpatterns}.
...
habían diseñado las features: primero la generación procedural del mundo, después la
integración del jugador y, finalmente, la incorporación de enemigos y control de encuentros.

\subsection{Implementación de Generación Procedural I}
...
\cite{garnetBindingMap,luiscanaryProcgen}.
...
El resultado de esta primera implementación puede observarse en la Figura~\ref{fig:iteracion1-feature-generacion}.

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=.7\textwidth]{figures/resultados/iteracion1/01_generacion_procedural.png}
\caption{Resultado de la feature Generación Procedural en la iteración 1}
\label{fig:iteracion1-feature-generacion}
\end{center}
\end{figure}

\subsection{Implementación de Jugador}
...
La Figura~\ref{fig:iteracion1-feature-jugador} muestra el resultado visual de esta integración del jugador.

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=.7\textwidth]{figures/resultados/iteracion1/02_anadir_jugador.png}
\caption{Resultado de la feature Jugador en la iteración 1}
\label{fig:iteracion1-feature-jugador}
\end{center}
\end{figure}

\subsection{Implementación de Enemigos}
...
El resultado obtenido para esta feature puede verse en la Figura~\ref{fig:iteracion1-feature-enemigos}.

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=.7\textwidth]{figures/resultados/iteracion1/03_anadir_enemigo.png}
\caption{Resultado de la feature Enemigos en la iteración 1}
\label{fig:iteracion1-feature-enemigos}
\end{center}
\end{figure}

\section{Pruebas}
Las pruebas realizadas en esta iteración se centraron en validar sobre partida real las
features desarrolladas. La Tabla~\ref{tab:iteracion1-pruebas} resume los playtests manuales ejecutados sobre la
generación procedural, el jugador y los enemigos, con el objetivo de comprobar tanto el
comportamiento esperado como la estabilidad de la base jugable mínima.

Además, estas pruebas también sirvieron para localizar errores concretos en la generación
procedural y en la integración inicial del jugador dentro del mapa. A partir de ellas se
ajustaron problemas de puertas, de instanciación del personaje y de colocación de ciertas
salas especiales, reforzando la estabilidad de la primera base jugable.

\begin{table}[!htbp]
    \centering
    \begin{coolTable}{p{.27\textwidth}p{.62\textwidth}}{2}
    {Pruebas de playtest realizadas en la iteración 1}
    \textbf{Feature bajo Test} & \textbf{Descripción de la prueba realizada} \\
    \midrule
    Generación Procedural & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Iniciar varias partidas desde la escena principal. \item Recorrer la sala inicial y avanzar por las puertas disponibles. \item Comprobar que las habitaciones generadas quedan conectadas, que no aparecen huecos bloqueantes y que las salas especiales se instancian sin romper la navegación.\end{enumerate}\end{minipage} \\
    \midrule
    Jugador & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Aparecer en la sala inicial de una mazmorra generada. \item Desplazarse en las cuatro direcciones y atravesar varias habitaciones. \item Verificar que el movimiento responde correctamente, que las colisiones impiden atravesar muros y que la escala del personaje resulta coherente con el escenario.\end{enumerate}\end{minipage} \\
    \midrule
    Enemigos & \begin{minipage}[t]{\linewidth}\begin{enumerate}\item Entrar en una sala con enemigos. \item Observar la activación del encuentro, el cierre de puertas y el cambio de los enemigos de reposo a persecución y ataque. \item Derrotar a todos los enemigos y comprobar que la sala libera de nuevo las puertas para continuar la exploración.\end{enumerate}\end{minipage} \\
    \end{coolTable}
    \caption{Pruebas de playtest realizadas en la iteración 1.}
    \label{tab:iteracion1-pruebas}
\end{table}

\section{Seguimiento y control}
El seguimiento de esta primera iteración se centró en comprobar si el alcance mínimo previsto
para arrancar el proyecto podía completarse con un nivel suficiente de calidad y dentro del
tiempo disponible. En términos funcionales, el resultado fue positivo: la iteración dejó una
base jugable con generación del mapa, jugador controlable y primeros encuentros en salas, lo
que permitió validar que la dirección técnica elegida era viable. Además, el incremento
cumplía de forma razonable la \textit{Definition of Done}, ya que el proyecto compilaba en
Unity sin errores críticos, las nuevas piezas estaban integradas en la partida y la revisión
cruzada junto con los \textit{playtests} de la Tabla~\ref{tab:iteracion1-pruebas} confirmaron
que el flujo principal funcionaba sin regresiones bloqueantes.

Las pruebas ejecutadas sirvieron no solo para verificar que el mapa se generaba, que el
jugador podía moverse y que los enemigos aparecían correctamente, sino también para detectar
que la generación procedural todavía necesitaba más consistencia en ciertas salas especiales.
Por ello, aunque el comportamiento observado fue satisfactorio como base funcional, se tomó la
decisión de no dar por completamente cerrada esa \textit{feature}. El principal riesgo
materializado en esta fase fue precisamente la complejidad real de la generación, superior a
la prevista inicialmente, aunque sin llegar a bloquear la iteración.

Desde el punto de vista temporal, la planificación fue bastante ajustada. El resumen de horas
puede consultarse en la Tabla~\ref{tab:iteracion1-horas}: frente a las 50:00 horas previstas,
la iteración consumió 50:17 horas reales, por lo que la desviación fue mínima y no obligó a
replantear el cronograma general. Sí se aplicó, en cambio, un pequeño ajuste de alcance:
aceptar el núcleo mínimo jugable y desplazar al backlog de la iteración 2 el refinamiento de
la generación procedural en su capa de prefabs e integración espacial. Ese refinamiento
incluía revisar habitaciones de 16x9 tiles, corregir offsets de habitaciones y puertas,
cerrar muros donde no hubiera puertas, ajustar colisiones y preparar una estructura más
modular para seguir incorporando nuevas \textit{rooms} sin degradar la consistencia del mapa.
Con ello, el estado técnico final quedó suficientemente estabilizado para continuar el
proyecto y la siguiente iteración pudo centrarse en consolidar esa base e introducir
progresión por círculos y cámara del jugador con menor riesgo de integración.

\begin{table}[!htbp]
    \centering
    \begin{coolTable}{lr}{2}
    {Horas de la iteración 1}
    \textbf{Horas planificadas de la iteración} & 50:00 \\
    \textbf{Horas reales (Adolfo Borrego González)} & 23:46 \\
    \textbf{Horas reales (Germán Sánchez Carmona)} & 26:31 \\
    \textbf{Horas reales totales de la iteración} & 50:17 \\
    \textbf{Horas acumuladas planificadas hasta esta iteración} & 150:00 \\
    \textbf{Horas acumuladas reales hasta esta iteración} & 147:05 \\
    \end{coolTable}
    \caption{Resumen de horas reales de la iteración 1.}
    \label{tab:iteracion1-horas}
\end{table}

La Figura~\ref{fig:uml-resultado-iteracion1} resume el diagrama UML de resultado de la iteración 1.
