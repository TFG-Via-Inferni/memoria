...
\begin{itemize}
    ...
    revisiones de código (\textit{pull requests}), las incidencias técnicas, los comentarios
    sobre cambios de arquitectura y las referencias a decisiones de diseño quedan
    registradas de forma permanente, permitiendo reconstruir el razonamiento detrás de cada
    cambio.
...
\begin{enumerate}
    ...
    revisión técnica por parte del otro desarrollador. La revisión se registra en GitHub
    mediante un \textit{pull request}, dejando trazabilidad de qué fue aprobado y por qué.
    En la revisión se valida decisiones de arquitectura, limpieza del código y coherencia
...
Complementariamente, se mantienen métricas simples durante el desarrollo:

\begin{itemize}
    \item \textbf{Tasa de regresiones:} Se registra si cada nueva iteración introduce
    bugs que rompen features previas. Una tasa alta indica problemas de integración que
    deben abordarse con refactorización o cambios arquitectónicos.

    \item \textbf{Estabilidad del compilado:} Se verifica que el juego compilado es estable
    y no crashea en escenarios típicos de uso. Los crashes son bloqueadores absolutos para
    la aceptación de features.

    \item \textbf{Coherencia en sesiones de juego extendidas:} Se realizan partidas largas
    del juego para verificar que la estabilidad se mantiene durante mucho tiempo y que no
    aparecen efectos secundarios como memory leaks o comportamientos erróneos tras varias
    horas de ejecución.
\end{itemize}
