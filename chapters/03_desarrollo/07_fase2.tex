...
\subsection{Feature: Sistema de Combate}
Esta feature regula la forma en la que el jugador ataca, aplica daño y expresa diferencias reales entre armas.

\begin{enumerate}
\item \textbf{Issue: Sistema de Combate (I) -- Iteración 5.} Se construyó la base del combate apoyada en armas cuerpo a cuerpo y a distancia.
\item \textbf{Issue: Sistema de Combate (II) -- Iteración 7.} Se refinó el sistema con ataques cargados, proyectiles más ricos en comportamiento y mejor retroalimentación visual.
\end{enumerate}

La separación entre ambas entregas permitió primero consolidar el combate como sistema funcional y, más adelante,
ampliarlo cuando ya existía contexto suficiente para introducir variantes de ataque y una integración más profunda
con otros sistemas.

\subsection{Feature: Sistema de Enemigos}
Esta feature gestiona el comportamiento táctico de los enemigos, su activación y su relación con el combate del jugador.

\begin{enumerate}
\item \textbf{Issue: Sistema de Enemigos (I) -- Iteración 5.} Se estableció la base de tipos enemigos y su integración inicial con el combate.
\item \textbf{Issue: Sistema de Enemigos (II) -- Iteración 7.} Se reforzó la diferenciación táctica entre arquetipos, la lectura visual del ataque y la respuesta al contexto del arma utilizada.
\end{enumerate}

La división fue necesaria porque el primer cierre de la feature aseguró la funcionalidad base, mientras que la segunda
entrega permitió convertir ese comportamiento en un sistema más legible y variado dentro de la partida.

\subsection{Feature: Actualizar Sprites}
Esta feature recoge las revisiones visuales realizadas para mejorar la legibilidad y la cohesión estética del proyecto.

\begin{enumerate}
\item \textbf{Issue: Actualizar Sprites (I) -- Iteración 5.} Se ajustó la presentación animada del jugador para mejorar la lectura visual del movimiento y su coherencia con el control.
\item \textbf{Issue: Actualizar Sprites (II) -- Iteración 8.} Se plantea una segunda revisión visual orientada a unificar HUD, menús y estados finales de victoria y derrota.
\end{enumerate}

La separación en dos issues responde a que no se trata de un ajuste aislado, sino de dos momentos distintos del desarrollo.
La primera entrega corrigió la base visual del jugador durante el núcleo jugable, mientras que la segunda se centra en el
acabado global de la experiencia y en su cierre visual.
...
\item \textit{Sala de tienda} (Iteración 7).
\item \textit{Sala secreta y de tesoro} (Iteración 7).
\item \textit{Victoria} (Iteración 8).
\item \textit{Derrota} (Iteración 8).
\end{itemize}
...
\newpage
\input{chapters/03_desarrollo/iteraciones/iteracion0}
\newpage
...
\newpage
\input{chapters/03_desarrollo/iteraciones/iteracion8}
